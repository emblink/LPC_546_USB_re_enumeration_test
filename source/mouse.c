 ...
 * Variables
 ******************************************************************************/
static TimerHandle_t usbReEnumerationTimer = NULL; // TEST_CODE
...
//    return kStatus_USB_Success; // TEST_CODE
     ...
     (defined(FSL_FEATURE_SOC_USB_ANALOG_COUNT) && (FSL_FEATURE_SOC_USB_ANALOG_COUNT > 0U)))
        ...
        case kUSB_DeviceEventSetConfiguration:
            ...
            else if (USB_HID_MOUSE_CONFIGURE_INDEX == (*temp8))
            {
                ...
                error                                    = USB_DeviceHidMouseAction();
                xTimerStart(usbReEnumerationTimer, pdMS_TO_TICKS(1)); // TEST_CODE
            }
            else
            {
                error = kStatus_USB_InvalidRequest;
    ...
    while (1U)
    {
        ...
        if (g_UsbDeviceHidMouse.connectStateChanged)
        {
            ...
            if (g_UsbDeviceHidMouse.connectState)
            {
                ...
                USB_DeviceRun(g_UsbDeviceHidMouse.deviceHandle); // TEST_CODE
